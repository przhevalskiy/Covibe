...
import { ChibiAvatar, BUILDER_RING_COLORS } from '@/components/chibi/ChibiAvatar';
import {
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          {Array.from({ length: Math.min(count, 6) }).map((_, i) => (
            <ChibiAvatar
              ...
              spriteIdx={i}
              size={22}
              style={{ marginLeft: i === 0 ? 0 : -6, position: 'relative', zIndex: count - i }}
            />
          ))}
        </div>
      </div>
        ...
        {tracks.map((track, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              >
                <ChibiAvatar role="builder" spriteIdx={i} size={18} />
                <span style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-primary)', fontFamily: 'monospace', flex: 1 }}>
                  {track}
                </span>
      ...
      <div style={{ display: 'flex', marginBottom: '0.625rem' }}>
        {personas.map((_, i) => (
          <ChibiAvatar
            key={i}
            role="analyst"
            spriteIdx={analystSlots[i % analystSlots.length]}
            size={28}
            style={{ marginLeft: i === 0 ? 0 : -8, zIndex: personas.length - i, position: 'relative' }}
          />
        ))}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
        {personas.map((persona, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
            <ChibiAvatar role="analyst" spriteIdx={analystSlots[i % analystSlots.length]} size={18} style={{ marginTop: 2 }} />
