...
import type { SwarmRole } from '@/components/chibi/agentTheme';
import { AGENT_ROLE_ACCENT } from '@/components/chibi/agentTheme';
...
export function agentSpriteIdxByType(type: AgentType, index: number): number {
  if (type === 'analyst') {
    const slots = [5, 9, 10, 1, 4, 11];
    return slots[index % slots.length];
  }
  if (type === 'builder') {
    const slots = [2, 6, 5, 10];
    return slots[index % slots.length];
  }
  return index;
}
...
export const ROLE_ACCENT: Record<string, string> = AGENT_ROLE_ACCENT;
